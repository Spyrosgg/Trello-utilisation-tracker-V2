...
var EFFORT_KEY = 'effortDays'; // card-scoped, shared plugin data: { memberId: daysPerWeek }
...
// Raw sum of the days belonging to members currently on the card.
// Stale entries (a member removed from the card) are ignored.
function sumDays(effortMap, memberIds) {
...
// Converts a total-days figure into "% of combined capacity" for whichever
// group of members it's being measured against, purely to drive badge color.
function percentOfCapacity(totalDays, memberCount) {
  if (!memberCount) return 0;
  return (totalDays / (WEEKLY_CAPACITY_DAYS * memberCount)) * 100;
}
...
// whether any shared member's own raw days add up past their weekly
  ...
  return t.cards('id', 'start', 'due', 'members').then(function (allCards) {
    ...
    return Promise.all(
      ...
      var overloadedIds = Object.keys(totals).filter(function (id) {
        return totals[id] > WEEKLY_CAPACITY_DAYS;
      });
      ...
      return overloadedIds.map(function (id) {
        return idToName[id] + ' (' + round1(totals[id]) + 'd/wk)';
      });
    });
  });
}
...
TrelloPowerUp.initialize({
  ...
  // Small badge on the front of the card: total days across assigned members
  'card-badges': function (t, opts) {
    return t.card('members').then(function (card) {
      ...
      return t.get('card', 'shared', EFFORT_KEY, {}).then(function (effort) {
        var totalDays = sumDays(effort, memberIds);
        if (totalDays <= 0) return [];
        var percent = percentOfCapacity(totalDays, memberIds.length);
        return [{
          text: round1(totalDays) + 'd',
    ...
    return Promise.all([
      ...
      var badges = [{
        ...
        text: memberIds.length
          ? round1(totalDays) + ' days total (' + Math.round(percent) + '% of capacity)'
          : 'Assign members',
        color: badgeColor(percent),
        callback: function (t) {
          return t.popup({
      ...
      return checkConcurrentOverload(t, card, effort).then(function (overloadedNames) {
        if (overloadedNames.length > 0) {
          badges.push({
            ...
            text: overloadedNames.join(', ') + ' over ' + WEEKLY_CAPACITY_DAYS + 'd/week this period',
            color: 'red'
          });
        }
        return badges;
